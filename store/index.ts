...
export const actions = {
  ...
  async sendTransaction({commit}, formData) {
    try {
      if (!ethereum) return alert("Please install metamask!");
      ...
      if(transactionHash) this.commit('settings/setIsLoading'); // setIsLoading(false);

      const transactionCount = await transactionContract.getTransactionCount();
      this.commit('setTransactionCount')
      this.dispatch('getTransactions', transactionCount.toNumber());
    } catch (error) {
      throw new Error("No ethereum object!");
    }
  }
